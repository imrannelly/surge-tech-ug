  ...
  */
  ...
  const normalizeProducts = (products) => (products || []).filter(Boolean).map((product) => {
    ...
    return {
      ...
      source: product.source || (product.dbId || product.db_id || product.uuid ? "supabase" : "local"),
  ...
  async function loadProducts(includeInactive = false) {
    ...
    console.info("Surge admin/catalog product source", {
      ...
      supabaseOnline: state.supabaseOnline,
      includeInactive,
    });
  ...
  async function saveAdminProduct(product) {
    ...
    if (supabase) {
      ...
      const saveByLocalId = async () => {
        ...
        console.info("Surge admin product insert/sync result", { data: response.data, error: response.error });
        return { data: response.data || [], error: response.error };
      };
      if (normalized.dbId) {
        const update = await supabase.from("products").update(row, { count: "exact" }).eq("id", normalized.dbId);
        console.info("Surge admin product id update result", { data: update.data, count: update.count, error: update.error });
        error = update.error;
        if (!error && update.count > 0) {
          const response = await supabase.from("products").select("*").eq("id", normalized.dbId).limit(1);
          data = response.data || [];
      ...
      if (!error && (!Array.isArray(data) || !data.length)) {
        const finalLookup = await supabase.from("products").select("*").eq("local_id", row.local_id).limit(1);
        console.info("Surge admin product final lookup response", { data: finalLookup.data, error: finalLookup.error });
        data = finalLookup.data || [];
        error = finalLookup.error;
      }
